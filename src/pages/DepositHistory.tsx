...
const DepositHistory: React.FC = () => {
  ...
  const deposits = user?.transactions?.filter((txn) => txn.type === "deposit") || [];
    ...
    <div className="animate-pulse">
      <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
        <thead className="bg-gray-100">
          <tr>
            {["TXN ID", "Amount", "Method", "Status"].map((head) => (
              <th
                key={head}
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold text-gray-800 mb-4">Deposit History</h1>
            ...
            <tbody>
              {deposits.map((txn) => (
                <tr key={txn.id || Math.random()} className="border-b">
                  <td className="py-2 px-4 text-sm text-gray-700">DEP{txn.id || "New"}</td>
